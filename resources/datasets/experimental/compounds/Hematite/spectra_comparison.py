"""Plot the unique experimental Fe K-edge spectra of hematite in this folder.
...
MEASUREMENTS = [
    (
        "M1  BMM, NSLS-II, RT",
        "NIST/Fe-Hematite.xdi",
        {"ratio": (4, 5), "ref_ratio": (5, 6)},
        "#0072B2",
    ),
    (
        "M2  IDEAS, CLS, RT",
        "XASDB/Hematite_idal8hze.dat",
        {"ratio": (1, 2), "ref_ratio": (2, 3)},
        "#E69F00",
    ),
    (
        "M3  BM08, ESRF, 300 K",
        "XASDB/Hematite_idjf3d2r.dat",
        {"mu_column": 1, "drop_last": True},
        "#D55E00",
    ),
    (
        "M4  20-BM, APS, RT",
        "XASDB/Iron (III) Oxide_idgbfxgi.dat",
        {"ratio": (1, 2)},
        "#CC79A7",
    ),
    (
        "M5  BM02-FAME, ESRF, RT",
        "FAME/EXPERIMENT_DT_20170706_002-SPECTRUM_DT_20170706_002/XAS_trans_Fe2O3/XAS_trans_Fe2O3.data.txt",
        {"fame": True},
        "#009E73",
    ),
    (
        "M6  BM08-GILDA, ESRF, 300 K, processed",
        "XASDB/Hematite_idslj4k7.dat",
        {"mu_column": 1, "ref_column": 2, "drop_last": True},
        "#000000",
    ),
]
...
def normalized(
    ...
    if fame:
        # The FAME data.txt starts with a metadata block, so keep only the
        # numeric rows. The transmission(au) column is already absorbance-like.
        with open(path) as handle:
            rows = [ln for ln in handle if ln.split() and ln.split()[0][:1].isdigit()]
        ...
        if ref_column is not None:
            ref_mu = data[:, ref_column]
        elif ref_ratio is not None:
            ref_mu = np.log(data[:, ref_ratio[0]] / data[:, ref_ratio[1]])
        else:
            ref_mu = None
        if ref_mu is not None and drop_last:
            ref_mu = ref_mu[:-1]
