"""Plot the experimental Fe K-edge spectrum of troilite in this folder.
...
MEASUREMENTS = [
    ("M1  SXRMB, CLS, RT", "XASDB/Iron (II) Sulfide_idozq8kn.dat", {"fluor_cols": (1, 3)}, "#0072B2"),
]

PRE_EDGE_DEFAULTS = {"pre1": -150, "pre2": -30, "norm1": 150, "norm2": 800, "nnorm": 2, "nvict": 2}


def normalized(path, fluor_cols=None):
    """Return a larch group with the normalized spectrum read from path."""
    data = np.loadtxt(path, comments="#")
    energy = data[:, 0]
    mu = data[:, fluor_cols[1]] / data[:, fluor_cols[0]]
