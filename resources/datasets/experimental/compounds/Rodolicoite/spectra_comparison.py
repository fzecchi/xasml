"""Plot the experimental Fe K-edge spectra of rodolicoite (FePO4) in this folder.
...
MEASUREMENTS = [
    ("M1  BMM, NSLS-II, RT", "NIST/Fe-IronPhosphate.xdi", {"ratio": (4, 5), "ref_ratio": (5, 6)}, "#0072B2"),
    ("M2  BL14B2, SPring-8, RT (axis unverified)", "MDR/Fe-K_FePO4_Si111_50ms_200526.dat", {"spring8": True}, "#D55E00"),
]

# Si(111) d spacing written in the SPring-8 file header, used to convert the
# monochromator angle to energy.
SPRING8_D_SPACING = 3.13551

FE_K_EDGE = 7112.0
PRE_EDGE_DEFAULTS = {"pre1": -150, "pre2": -30, "norm1": 150, "norm2": 800, "nnorm": 2, "nvict": 2}


def read_spring8(path):
    """Return energy and mu from a raw SPring-8 BL14B2 transmission file."""
    rows = []
    ...
    data = np.array(rows[1:])  # the first numeric row is the block table
    angle = np.radians(data[:, 1])  # Angle(o), the read-back angle
    energy = 12398.0 / (2 * SPRING8_D_SPACING * np.sin(angle))
    mu = np.log(data[:, 3] / data[:, 4])
    order = np.argsort(energy)
    return energy[order], mu[order]


def normalized(path, ratio=None, ref_ratio=None, spring8=False):
    """Return a larch group with the normalized spectrum read from path."""
    if spring8:
        energy, mu = read_spring8(path)
